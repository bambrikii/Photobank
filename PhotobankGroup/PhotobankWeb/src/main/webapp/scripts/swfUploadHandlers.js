...
function uploadSuccess(file, serverData) {
	try {
		...
		if (serverData.substring(0, 3) === "id:") {
			addLink(file.name + ", " + serverData.substring(3), this.customSettings.result_target);
	...
	try {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
			try {
				...
				progress.toggleCancel(false);
				addFailure(file.name + ". Cancelled.", this.customSettings.result_target)
			} catch (ex1) {
				this.debug(ex1);
			}
			break;
		case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
			try {
				...
				addFailure(file.name + ". Stopped.", this.customSettings.result_target)
			} catch (ex2) {
				this.debug(ex2);
			}
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			imageName = "uploadlimit.gif";
			addFailure(file.name + ". Upload limit exceeded.", this.customSettings.result_target)
			break;
		default:
			addFailure(file.name + ". " + message + "(" + errorCode + ").", this.customSettings.result_target)
			break;
		}
	} catch (ex3) {
		this.debug(ex3);
...
function addLink(src, targetId) {
	var div1	= document.createElement("div");
	var list	= document.getElementById(targetId);
	div1.style.margin	= "5px";
	var a1	= document.createElement("a");
	var txt1 = document.createTextNode(src);
	a1.appendChild(txt1);
	div1.appendChild(a1);
	list.insertBefore(div1, list.firstChild);
}

function addFailure(src, targetId) {
	var div1	= document.createElement("div");
	var list	= document.getElementById(targetId);
	div1.style.margin	= "5px";
